...
const Chart = ({ className = '' }) => {
  ...
  const data = allCases.reduce<{ date: string; data: CaseItem[] }[]>((accumulator, currentValue) => {
    ...
    } else if (accumulator[accumulator.length - 1].date === currentValue.date) {
  ...
  const graphColX = data.map((item) => moment(item.date).format('YYYY-MM-DD'));
  const graphColCase = data.map((item) => item.data.length);
  const graphColCumulative = data.reduce<number[]>((acc, current) => {
    if (!acc.length) {
